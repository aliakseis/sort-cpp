...
void HungarianAlgorithm::assignmentoptimal(int *assignment, double *cost, double *distMatrixIn, int nOfRows, int nOfColumns)
{
	...
	*cost = 0;
	...
	distMatrix = static_cast<double *>(malloc(nOfElements * sizeof(double)));
	...
	/* memory allocation */
	coveredColumns = static_cast<bool *>(calloc(nOfColumns, sizeof(bool)));
	coveredRows = static_cast<bool *>(calloc(nOfRows, sizeof(bool)));
	starMatrix = static_cast<bool *>(calloc(nOfElements, sizeof(bool)));
	primeMatrix = static_cast<bool *>(calloc(nOfElements, sizeof(bool)));
	newStarMatrix = static_cast<bool *>(calloc(nOfElements, sizeof(bool))); /* used in step4 */
